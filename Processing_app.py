...
def extract_metadata(full_text, field, api_key):
    ...
    if not isinstance(full_text, str):
        st.error(f"Expected string for full_text, got {type(full_text)}")
        return None
    ...
    try:
        ...
        st.error(f"Error in extract_metadata for field '{field}': {e}")
        return None
...
def process_pdf(uploaded_file, api_key):
    ...
    try:
        ...
        # Process document - ensure full_text is a string
        full_text = "\n".join(pages_text)
        
        # Validate that full_text is indeed a string
        if not isinstance(full_text, str):
            st.error(f"Error: full_text is not a string, it's {type(full_text)}")
            return None
        ...
        for field in metadata_fields:
            try:
                ...
                st.error(f"Error extracting {field}: {e}")
                metadata[field] = None
        ...
        try:
            ...
            headers_structure = ["Document"]
        
        try:
            document_segments = segment_document(full_text, headers_structure)
        except Exception as e:
            st.error(f"Error segmenting document: {e}")
            document_segments = {"Full Document": full_text.split('\n')}
...
def main():
    ...
    if st.session_state.processed_data:
        ...
        with col1:
            preview_btn = st.button("🖼️ Preview Pages", use_container_width=True)
        ...
        if preview_btn:
            st.subheader("🖼️ PDF Pages Preview")
